...
- **Rotas (atualizado na T-091a)**: o dashboard é `/investimentos/acoes` (sem param) — desde a T-091a ele é filho do guarda-chuva **Investimentos** (`/investimentos`, irmão de Cripto e Renda Fixa), e não mais um layer plano em `/dash`. `/dash`, `/dash/:id` e `/carteiras` respondem `<Navigate to="/investimentos/acoes" replace />` — bookmarks antigos não quebram. Os paths da árvore vivem só em `packages/web/src/routes/investmentsTree.ts`. Na Home não há mais card "Ações": o card **"Investimentos"** aponta para o hub, e o valor que ele exibe continua sendo o da carteira B3 (`computeStockTotals`), inalterado.
