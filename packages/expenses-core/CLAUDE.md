...
Extraído de `packages/server/src/api/services/recurringExpenses.ts` na T-099b
...
Rotas: `packages/server/src/api/routes/{expenses,expenseEntries,recurringExpenses,budgets}.ts`.
...
Decisão do humano (T-037, 2026-07-25): a seção "Orçamento do mês" (barras de progresso, form de criação/upsert e botão de remover) foi removida do render de `DespesasPage.tsx` — "não entendi a utilidade do orçamento do mês". O backend seguiu intacto: `packages/server/src/api/routes/budgets.ts`, `packages/web/src/routes/budgetProgress.ts` e todos os testes de ambos continuaram ativos.
