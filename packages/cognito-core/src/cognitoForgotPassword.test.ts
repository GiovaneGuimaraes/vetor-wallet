...
import { cognitoForgotPassword } from './cognitoForgotPassword';
...
describe('cognitoForgotPassword (T-108a)', () => {
  it('manda ClientId e Username normalizado, e não devolve o destino mascarado', async () => {
    ...
    const result = await cognitoForgotPassword(' Alice@Example.com ');
  ...
  it('pool COM secret: SecretHash na raiz do corpo', async () => {
    ...
    await cognitoForgotPassword('alice@example.com');
  ...
  it('e-mail sem cadastro no pool vira userNotFound (a rota é quem mascara)', async () => {
    ...
    const err = await cognitoForgotPassword('ninguem@example.com').catch((e) => e);
  ...
  it('throttling do Cognito vira tooManyRequests', async () => {
    ...
    const err = await cognitoForgotPassword('a@b.com').catch((e) => e);
