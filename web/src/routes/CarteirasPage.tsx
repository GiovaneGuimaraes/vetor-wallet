 ...
 * page de várias carteiras"). A decisão de criar/redirecionar/listar é pura
 * (`decideWalletFlow`, testada em `walletFlow.test.ts`); esta página só
 * interpreta o resultado:
 *
 * - 0 carteiras → cria a "Principal" automaticamente via `POST /api/wallets`
 *   (rota já existente, sem mudança no server) e, assim que `wallets` for
 *   atualizado pelo `ShellContext`, a decisão vira `redirect` sozinha.
...
export function CarteirasPage() {
  const { user, wallets, walletsLoaded, walletSummaries, onSelectWallet, onCreateWallet, theme } = useShellContext();
  ...
  useEffect(() => {
    ...
    const decision = decideWalletFlow(wallets, forceList);
    ...
    if (decision.action === 'create' && !creatingRef.current) {
  ...
  }, [walletsLoaded, wallets, forceList, navigate, onCreateWallet]);
