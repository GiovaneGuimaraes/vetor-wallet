...
- **Segue sem decisão** (não perguntar de novo junto do resto — são independentes): as opções 2️⃣ (transferência entre contas próprias e pagamento de fatura **não** importar) e 3️⃣ (caixa de entrada de revisão). Enquanto não houver decisão, **a importação real da Pluggy segue proibida sem `--dry-run`** — o pagamento de fatura contando duas vezes não depende do caso `Investments`.
