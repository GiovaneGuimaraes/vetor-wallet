...
- **O plano completo**: [`plano-migracao-aws.md`](./plano-migracao-aws.md) — **reescrito em 2026-09-20**: o AppSync e o Relay saíram, e o transporte passou a ser API Gateway + Lambda com o REST de hoje preservado.
- **Decisão 1 — para onde vai o banco: RESPONDIDA em 2026-09-14 — 2️⃣ Aurora Serverless v2 + Postgres.** Eu recomendava a 1️⃣ (Turso) por ser a única sem reescrita de SQL; o humano escolheu a 2️⃣ com os dois custos na mesa — **reescrever o SQL dos cores** (176 pontos, medidos) e **pagar capacidade mínima contínua**. Duas razões a favor que a minha recomendação subestimava: é a pilha que ele já opera na OCA, e Postgres **devolve o desenvolvimento offline** (container local), que eu havia dado como perdido.
- **Premissa corrigida no caminho**: a AppSync **não exige** Postgres — ela alcança qualquer banco por Lambda resolver; o que ela não alcança é o arquivo SQLite no disco. E o desenho da OCA nem usa resolver direto, então a vantagem "sem Lambda" da Aurora não se aplica aqui.
- **Referência adotada** (indicada pelo humano em 2026-09-14): o monorepo privado `oneclickads/monorepo` — `postgresdb` (modelos + `sync`), `lambda-postgres-query` (proxy na VPC), `query` (`{ text, values }` injetado nos cores, igual ao formato-alvo da T-104), `graph-api` (schemaComposer + um handler AppSync), `app` com `react-relay`. **Nenhuma ARN, id de conta ou hostname de lá entra neste repo — ele é público.**
- **Decisão 2 — infra as code: RESPONDIDA em 2026-09-20 — scripts em SDK puro da AWS (`@aws-sdk/client-*`), com o `carlin` registrado como migração futura.** Eu recomendava `carlin`, por ser a pilha que ele já opera na OCA; ele escolheu o SDK **com o custo na mesa** e por um motivo que a minha recomendação não pesava: **quer ver como o deploy funciona por baixo** — provisionar a AppSync na mão é objetivo, não efeito colateral. O que isso custa e as três regras que mantêm a conta pagável estão na **§8.1 do plano**: estado no **SSM Parameter Store** (id de infra não entra neste repo público), scripts **idempotentes** ("descreve → cria ou atualiza") e **um recurso = uma função `ensureX` pura** no formato da T-104. A porta para o `carlin` fica aberta enquanto **nenhum código do app importar `packages/infra`** — é a única invariante a vigiar.
- **Decisão 3 — o que substitui a sessão em SQLite: ABERTA, e mudou de forma.** Com o AppSync fora, não é mais "token em memória × BFF": o Express **continua existindo**, então o BFF deixou de ser peça extra. Virou **(1️⃣) trocar o store da sessão para Postgres** × **(2️⃣) trocar a sessão por JWT do Cognito validado no API Gateway**. **Recomendo a 1️⃣ para a fase 2 e a 2️⃣ como destino** — a 1️⃣ é troca de store, não de arquitetura: nada no web muda e o `change-password` (que guarda o access token na sessão do servidor, e já mordeu na T-092) não é reescrito no meio da migração de banco. Detalhe na §6.1 do plano. **Só é necessária na fase 2.**
- **Decisão 4 — teto de custo: RESPONDIDA em 2026-09-20 — o Budget foi criado pelo humano.** Era pré-requisito de qualquer recurso na AWS, porque a Aurora Serverless v2 cobra **enquanto o cluster existir**, com o app parado.
- **Decisão 5 — região: RESPONDIDA em 2026-09-20 — `us-east-1`**, a mesma do user pool do Cognito (T-106).
- **Decisão 6 — transporte da API: RESPONDIDA em 2026-09-20 — API Gateway + Lambda, com o REST de hoje preservado.** O humano avaliou que o app é simples e está no começo demais para pagar o preço do AppSync, e eu concordei com o raciocínio: o AppSync entrega linguagem de consulta, subscriptions e resolvers gerenciados, e **o app não usa nenhuma das três** — enquanto cobraria Relay no web, um schema em paralelo aos tipos de `shared/` e a regra de ownership reescrita resolver a resolver. Com API Gateway a migração vira "onde isso roda" em vez de "como isso é escrito", e os 440 testes de rota seguem valendo como rede de segurança. **O que se perde**: subscriptions (hoje é polling, então nada que exista) e uma requisição por tela (que já são duas hoje). A porta do AppSync continua aberta de graça, porque quem viabiliza os dois é o core com `db`/`query` injetado.
- **Resposta do humano**: _(decisão 1 respondida em 2026-09-14; faltam 2, 3 e 4)_
