...
- **Caixinhas são Renda Fixa, IRMÃS de Ações — não dentro dela** (decisão de engenharia, 2026-08-13). O humano pediu "dentro de Ações, perto da carteira"; a proximidade visual é mantida (irmãs no mesmo pai), mas pôr renda fixa *dentro* de Ações recriaria exatamente o acoplamento que esta tarefa existe para desfazer. Caixinha não é só apelido de poupança: no Nubank é lastreada em RDB e rende % do CDI, e o padrão não é exclusivo dele (cofrinho no PicPay/Inter, objetivos no C6).
...
- **Caixinha pode ser a sucessora natural de Metas** (para o humano decidir): meta é dinheiro carimbado para um objetivo com progresso manual; caixinha é o mesmo, com saldo real e rendendo. Se caixinha "herdar" as metas, ele não perde a função — ganha a versão com dinheiro de verdade.
