...
- **Status**: CONCLUIDA (2026-08-09) — feita direto pelo orquestrador a pedido do humano ("faça T-100 do renomear o server, porém mantenha a rest-api sem deploy"), entre a T-104a e a T-104b, como a nota abaixo recomendava. **Nenhuma configuração de deploy foi criada** — a decisão de 2026-08-08 (API só local) segue valendo e está registrada no `CLAUDE.md` do package.
  - `git mv packages/server packages/rest-api` (histórico preservado), package `vetor-wallet-server` → `vetor-wallet-rest-api`, scripts da raiz e `.gitignore` (`packages/rest-api/data/`) atualizados. `pnpm-workspace.yaml` usa o glob `packages/*` e não precisou mudar.
  - **O alias `@vetor-wallet/server/*` do `cli/tsconfig.json` já não existia** — saiu na T-099c. O escopo original da tarefa (escrito no Ciclo 19) ainda o listava; só a documentação do `cli` tinha ficado para trás, descrevendo um alias morto e a `services/` que também já não existe. Corrigida para a realidade: o CLI importa dos `*-core`.
  - **Armadilha real, e a única com risco de perda de dado**: o `.env` local do `cli` apontava `DATABASE_URL=file:../server/data/wallet.db`. Esse caminho **não dá erro** depois do rename — o libsql cria um banco novo e vazio, e o job roda "com sucesso" sobre nada. `.env` e `.env.example` do `cli` foram atualizados e o aviso ficou registrado no `packages/cli/CLAUDE.md`. O `data/wallet.db` e o `.env` da API (com a chave AbacatePay) acompanharam o `git mv` do diretório.
  - Verificações: `pnpm build`, `pnpm lint`, `pnpm format:check` e `pnpm test` verdes, com as contagens **idênticas** ao baseline (rest-api 452/25, web 454/28, subscription 131, validation 36, db 29, brapi 16, savings 30, expenses 18, bank-import 34, insights 23, portfolio 73, auth 10). Boot do entry compilado provado por execução (`node dist/api/index.js`), com o `snapshotScheduler` instrumentado — checagem exigida pela T-099c.
  - **Registro histórico preservado**: as menções a `packages/server` no `BACKLOG-ARQUIVO.md`, no `plano-t-084.md` e nas entradas de tarefas já concluídas **não** foram reescritas — descrevem o repo como ele era na época. Só os ponteiros vivos migraram (T-021, candidata da T-105, `TODO-HUMANO.md`). **Consequência — o critério de aceite original não foi cumprido ao pé da letra**: ele pedia "nenhuma ocorrência de `vetor-wallet-server` ou `@vetor-wallet/server` sobra no repo (grep)", e sobram, dentro de `docs/multi-agent/` e do `BACKLOG-ARQUIVO.md`. Reescrevê-las deixaria o grep limpo ao custo de falsificar o registro de decisões — que é justamente o que esses arquivos existem para guardar. O grep que **está** limpo, e que é o que importa, é o de **código e docs vivas**: nenhum `package.json`, tsconfig, config de teste, script, `.env`, `CLAUDE.md` ou `docs/decisions/` menciona o nome antigo. Quem repetir o grep original vai ver os matches históricos — eles são esperados.
- **Prioridade**: P3
- **Complexidade**: média
- **Depende de**: T-099
- **Branch/worktree**:
...
- **Status**: CONCLUIDA — PR #152 mergeado (2026-08-09). Executor Sonnet, revisor Sonnet. **REPROVADA 1×**, corrigida e APROVADA na re-revisão. Web 451 → **454** (28 arquivos), pelos 3 casos do `mascots.test.ts` novo.
  - Header com `/logo.png` fixo (o `key={mascotSrc}` que remontava a imagem a cada rota saiu); landing trocou `receitas-t.png` pelo logo oficial; as 5 pages restantes ganharam o próprio mascote a 48px (a `CriptoPage` já tinha o seu, não mudou).
  - **Os 3 achados da reprovação**, todos procedentes: (a) breakpoints divergentes — 480px via CSS próprio nas pages × 640px via `hidden sm:block` do Tailwind no Dashboard, misturando dois mecanismos para a mesma decisão visual; (b) a montagem de `` `/layers/${MASCOT_FILE_BY_LAYER...}` `` **copiada em 5 arquivos**, enquanto `mascotSrcForPathname` já fazia isso e tinha acabado de ficar órfã; (c) as duas funções órfãs mantidas sob a justificativa de "API pública do módulo" — fraca num app interno sem consumidor externo nem teste.
  - Correção: breakpoint unificado na classe `.vw-page-mascot`, helper `mascotSrcForLayer()` extraído **com teste ao lado** (primeiro `mascots.test.ts` do projeto), e `mascotSrcForPathname`/`layerFromPathname` removidas. Revisor confirmou os greps de forma independente na re-revisão.
  - **Lição de processo**: a primeira entrega passou em lint/format/build/testes e ainda assim estava errada — nenhum desses sinais pega inconsistência visual nem duplicação. Em tarefa de UI, o critério "mesmo tratamento nas N pages" precisa ser verificado lendo o código das N pages lado a lado, não pelo relatório do executor.
- **Prioridade**: P2
...
- **Causa — dois comportamentos corretos isoladamente que juntos abrem o buraco**:
  1. `packages/rest-api/src/api/routes/operations.ts:74-84` valida a venda contra a **posição de hoje** (`buildPositionMap` sobre TODAS as operações do ticker), ignorando a `date` informada. Como hoje há 100, passa.
...
- **Da T-020 (2026-08-09)**: três origens de mascote no web montam o caminho `/layers/<arquivo>` com mapas próprios — `mascots.ts` (helper `mascotSrcForLayer`, testado), `AuthPage.tsx:123` (`f.mascot`, lista de features) e `HomePage.tsx:303` (`card.mascot`). Só a primeira foi unificada na T-020; as outras duas são pré-existentes e ficaram fora de escopo. Candidata a um helper compartilhado único.
- **Da T-105 (2026-08-08)**: o glob `packages/*/{src,tests}/**` dos scripts `format`/`format:check` não alcança arquivos na **raiz** de cada package — `packages/rest-api/vitest.config.ts` segue fora do padrão Prettier e o step novo do CI não o detecta. Achado do revisor; expandir o glob é diff próprio.
