...
## Ciclo 14 — Colheita + polish + benchmarks no gráfico (2026-07-27, PRs #110–#114)

### T-064 — Índice em quote_snapshots(ticker, captured_at) — PR #110. Executor Sonnet, revisor Sonnet: APROVADA. O índice `idx_snapshots_ticker_time` já existia no schema.ts (entrou na T-063); a tarefa virou cobertura de teste: `db/schema.test.ts` novo verifica via sqlite_master existência e idempotência após initDb.

### T-065 — Sobras de revisão no server — PR #112. Executor Sonnet, revisor Sonnet: APROVADA. `MAX_MONEY_AMOUNT` (1e13) em `isValidMoneyAmount` + `moneyAmountError` propagado a ~10 rotas de dinheiro (mensagem não fala "casas decimais" num problema de limite); `withUserLock` (mutex por promise-chaining, `previous.then(fn,fn)` + cópia com catch no map) no POST /api/wallets contra o TOCTOU de count+INSERT, com teste concorrente real; re-SELECTs de alerts/wallets filtrados por `user_id` com testes de isolamento; rótulo `'price'` → `'preço'` no erro de casas decimais do import CSV. Sugestão não bloqueante: limpar entradas do map de locks no finally.

### T-066 — MIN_ABS_PADDING compartilhado + rótulos abreviados — PR #111. Executor Sonnet, revisor Sonnet: APROVADA. Novo `web/src/routes/chartAxisFormat.ts`: `MIN_ABS_PADDING` único (importado por chartGeometry/historyChart, comportamento preservado) + `formatAxisValue` abreviando mil/mi/bi em pt-BR nos rótulos do eixo Y (ProjectionChart/HistoryChart); `<desc>` acessíveis mantêm valor cheio. Sugestão não bloqueante: borda 999.950 rende "R$ 1000,0 mil" em vez de "R$ 1,0 mi".
