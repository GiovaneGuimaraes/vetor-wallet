...
CREATE TABLE IF NOT EXISTS recurring_expenses (
  ...
  start_month  TEXT    NOT NULL,   -- YYYY-MM; meses anteriores nunca são gerados
  active       INTEGER NOT NULL DEFAULT 1,
...
- **Criação sempre acoplada ao lançamento.** Não há `POST /api/recurring-expenses`; a recorrência nasce em `POST /api/expense-entries` com `recurring: true`. Isso resolve duas coisas de graça: `start_month` é sempre o mês do lançamento (sem ambiguidade de fuso) e a **primeira ocorrência é o próprio lançamento criado**, já registrada no livro-razão — sem isso o primeiro GET do mês de criação geraria uma segunda cópia idêntica. `dayOfMonth` opcional sobrepõe o dia derivado de `date`.
- **A idempotência é do banco, não do código.** `recurring_expense_months` tem `UNIQUE(recurring_id, month)`. A materialização faz `INSERT OR IGNORE` na reserva do mês e só insere a ocorrência quando `rowsAffected === 1`; o perdedor de uma corrida de dois GETs simultâneos do mesmo mês simplesmente não insere. Antes do laço, uma única query carrega os pares (recorrência, mês) já gerados — o `INSERT OR IGNORE` é a rede de segurança contra concorrência, não o caminho normal.
...
- **Meses futuros são materializados** (decisão): navegar ‹/› para frente em `/despesas` mostra a assinatura que já se sabe que vai cair lá — é o que o usuário espera ao planejar. `/summary` nunca gera mês futuro porque sua janela termina no mês corrente.
- **Meses anteriores a `start_month` nunca são gerados**: navegar para trás não inventa histórico.
