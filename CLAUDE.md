...
- **A reversão do "sem gráfico" (T-033/T-040) vale só para `/dash`**: o gráfico SVG do resultado da projeção é a T-057b, que entra no **mesmo card** (ponto de inserção marcado no JSX) — não é uma mudança de política geral do app, `/poupanca` continua sem gráfico.
...
- **Fora de escopo (segue pendente)**: gráfico (T-057b), barras de alocação (T-057c), aporte mensal recorrente, comparação com CDI/Ibovespa, endpoint de projeção no server.
...
- **Fora de escopo**: normalizar dados legados já gravados com mais de 2 casas (continuam lidos como estão) e o web, que já só envia 2 casas via `parseMoneyInput`.
